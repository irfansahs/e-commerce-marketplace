...
2. **Skeleton** — .NET 10 Web API project with:
   - `AddMarketplaceLogging("{service}")`, `UseMarketplaceCorrelationId()`, `MapMarketplaceHealth("{service}")`
   - Project reference to `services/building-blocks/Marketplace.BuildingBlocks`
   - `appsettings.json` + connection string placeholder for dedicated SQL Server DB (`Server=localhost,14330;Database={service}_db;User Id=sa;...`)
   - Dockerfile (multi-stage) when user asks for container deploy
   - Add project to root `Marketplace.slnx` under its `services/{service}` solution folder
