...
type RegisterBody = { email: string; password: string; locale?: string };

export async function POST(request: NextRequest) {
  const body = (await request.json()) as RegisterBody;
  const locale = body.locale ?? "tr";
  const preferredCulture = locale === "en" ? "en-US" : "tr-TR";

  const upstream = await fetch(gatewayUrl("/api/identity/auth/register"), {
    method: "POST",
    headers: {
